  ...
  -- 9b) Admin helper return type (fix migration 20260108161000)
  select
    'Admin helper (require_admin returns boolean)' as check_type,
    count(*) as found,
    case
      when count(*) >= 1 then '✓ OK'
      else '✗ MISSING (run 20260108161000)'
    end as status
  from pg_proc p
  join pg_namespace n on n.oid = p.pronamespace
  where n.nspname='private'
    and p.proname = 'require_admin'
